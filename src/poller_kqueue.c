...
static uint32_t translate_ready_event(const struct kevent *event) {
    uint32_t ready = CROUTINE_IO_EVENT_NONE;
    if (event->filter == EVFILT_READ) {
        ready |= CROUTINE_IO_EVENT_READ;
    }
    if (event->filter == EVFILT_WRITE) {
        ready |= CROUTINE_IO_EVENT_WRITE;
    }
    if ((event->flags & (EV_EOF | EV_ERROR)) != 0) {
        ready |= CROUTINE_IO_EVENT_READ | CROUTINE_IO_EVENT_WRITE;
    }
    return ready;
}
...
bool croutine_poller_set(int fd, uint32_t old_events, uint32_t new_events) {
    ...
    if (!sync_filter(fd, old_events, new_events, CROUTINE_IO_EVENT_READ,
                     EVFILT_READ)) {
        return false;
    }

    if (sync_filter(fd, old_events, new_events, CROUTINE_IO_EVENT_WRITE,
                    EVFILT_WRITE)) {
        return true;
    }

    (void)reverse_filter(fd, old_events, new_events, CROUTINE_IO_EVENT_READ,
                         EVFILT_READ);
    return false;
}
